...
bool HelloWorldPublisher::init()
{
    ...
    mp_participant = Domain::createParticipant(PParam);//PParam
    ...
    Wparam.topic.topicDataType = "HelloWorldData::Msg";
    Wparam.topic.topicName = "HelloWorldData_Msg";
    Wparam.topic.historyQos.kind = KEEP_LAST_HISTORY_QOS;
    Wparam.topic.historyQos.depth = 30;
    // Wparam.topic.resourceLimitsQos.max_samples = 50;
    // Wparam.topic.resourceLimitsQos.max_samples_per_instance = 2;
...
void HelloWorldPublisher::run(uint32_t samples)
{
    for(uint32_t i = 0;i<samples;++i)
    {
        ...
        {
            std::cout << "Message: "<<m_Hello.message()<< " with userID: "<< m_Hello.userID()<< " SENT"<<std::endl;
            usleep(5000*1000);
        }
    }
}
...
bool HelloWorldPublisher::publish()
{
    ...
    {
        m_Hello.userID(m_Hello.userID()+1);
